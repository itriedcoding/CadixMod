#!/bin/bash
# CadixMod macOS Installer
# Bash script to install CadixMod on macOS
...
install_cadixmod() {
    ...
    ASAR_PATH="$DISCORD_PATH/app.asar"
    BACKUP_PATH="$DISCORD_PATH/app.asar.backup"
...
EOF
    ...
    CADIXMOD_DIST="$DISCORD_PATH/cadixmod"
    ...
    if command -v codesign &> /dev/null; then
        write_warning "Note: You may need to re-sign Discord after installation."
        write_status "Run: codesign --deep --force --sign - /Applications/Discord.app"
    fi
}
